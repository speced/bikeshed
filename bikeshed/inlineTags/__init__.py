...
def processTags(doc):
	for el in findAll("[data-span-tag]", doc):
		...
		try:
			...
			die("When trying to process {0}, got invalid unicode in stdout:\n{1}", outerHTML(el), e, el=el)
		try:
			err = err.decode("utf-8")
		except UnicodeDecodeError as e:
			die("When trying to process {0}, got invalid unicode in stderr:\n{1}", outerHTML(el), e, el=el)
		if p.returncode:
			die("When trying to process {0}, got return code {1} and the following stderr:\n{2}", outerHTML(el), p.returncode, err, el=el)
			continue
