    ...
    def format(self, x: float, y: float, width: float) -> OptionalSequence:
        ...
        for i, item in enumerate(self.items):
            ...
            if i == 0:
                ...
                (Path(x + item.width + AR * 2, y + item.height).right(itemSpace + AR).addTo(self))
                # Lower skip
                (
                    Path(x, y)
                    .arc("ne")
                ...
                (Path(x, y).right(AR * 2).addTo(self))
                item.format(x + AR * 2, y, item.width).addTo(self)
                (Path(x + AR * 2 + item.width, y + item.height).right(itemSpace + AR).addTo(self))
                # Lower skip
                (
                    Path(x, y)
                    .arc("ne")
...
class Choice(DiagramMultiContainer):
    ...
    def format(self, x: float, y: float, width: float) -> Choice:
        ...
        if above:
            distanceFromY = max(AR * 2, default.up + VS + above[0].down + above[0].height)
        ...
        if below:
            distanceFromY = max(AR * 2, default.height + default.down + VS + below[0].up)
        for i, item in enumerate(below):
            ...
            distanceFromY += max(
                AR,
                item.height + item.down + VS + (below[i + 1].up if i + 1 < len(below) else 0),
            )
        return self
...
class MultipleChoice(DiagramMultiContainer):
    ...
    def format(self, x: float, y: float, width: float) -> MultipleChoice:
        ...
        above = self.items[: self.default][::-1]
        if above:
            distanceFromY = max(10 + AR, default.up + VS + above[0].down + above[0].height)
        for i, ni, item in doubleenumerate(above):
        ...
        below = self.items[self.default + 1 :]
        if below:
            distanceFromY = max(10 + AR, default.height + default.down + VS + below[0].up)
        for i, item in enumerate(below):
...
class HorizontalChoice(DiagramMultiContainer):
    ...
    def format(self, x: float, y: float, width: float) -> HorizontalChoice:
        ...
        # upper track
        upperSpan = (
            sum(x.width + (20 if x.needsSpace else 0) for x in self.items[:-1]) + (len(self.items) - 2) * AR * 2 - AR
        )
        (Path(x, y).arc("se").up(self._upperTrack - AR * 2).arc("wn").h(upperSpan).addTo(self))
        ...
        for [i, item] in enumerate(self.items):
            ...
            if i == 0:
                ...
                (Path(x, y - self._upperTrack).arc("ne").v(self._upperTrack - AR * 2).arc("ws").addTo(self))
                x += AR * 2
            ...
            if i == len(self.items) - 1:
                ...
                if item.height - self._lowerTrack >= AR * 2:
                    ...
                    (Path(x, y + item.height).l(AR * 2, self._lowerTrack - item.height).addTo(self))
            else:
                (Path(x, y + item.height).arc("ne").v(self._lowerTrack - item.height - AR * 2).arc("ws").addTo(self))
        return self
...
class OneOrMore(DiagramItem):
    def __init__(self, item: Node, repeat: Opt[Node] = None) -> None:
        ...
        self.down = max(AR * 2, self.item.down + VS + self.rep.up + self.rep.height + self.rep.down)
        self.needsSpace = True
        addDebug(self)
