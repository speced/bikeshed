...
def processSVGInclusions(doc):
    iters = 0
    while True:
        # Loop because an include can contain more includes.
        iters += 1
        if iters > 1000:
            die("Looked for include-svg blocks more than 1000 times, something is wrong.")
            return
        els = findAll("pre.include-svg", doc)
        if not els:
            break
        for el in els:
            handleBikeshedIncludeSVG(el, doc)

def handleBikeshedIncludeSVG(el, doc):
    macros = {}
    for i in itertools.count(0):
        m = el.get("macro-" + str(i))
        if m is None:
    ...
    if el.get("path"):
        ...
        text = ''.join(lines)
        text = doc.fixText(text, moreMacros=macros)
        subtree = parseSVG(text)
        for childInclude in findAll("pre.include-svg", E.div({}, *subtree)):
            childInclude.set("hash", hash)
            childInclude.set("depth", str(depth + 1))
        replaceNode(el, *subtree)
    else:
        die("Whoops, an include-svg block didn't get parsed correctly, so I can't include anything.")
        removeNode(el)
        return
