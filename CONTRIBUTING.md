...
* To run Playwright visual tests and tests of client-side user interaction,
    NodeJS and docker must be installed first.  Then just run:
    `npm run pwtests`
    This will run all *_pwtest.js scripts and compare with existing  ___screenshots__ in the same directory.  To update the screenshots do:
